...
export default function SelectSearchField({ label, required, helpText, value, onChange, options }: { label: string; required?: boolean; helpText?: string | null; value: string; onChange: (v:string)=>void; options: string[] | { label: string; value: string }[]; }) {
  ...
  const filtered = useMemo(() => flat.filter(o => o.label.toLowerCase().includes(q.toLowerCase())), [flat, q]);
        ...
        <div ref={panelRef} className="absolute z-10 mt-1 w-full rounded-md border border-gray-200 bg-white shadow-lg overflow-hidden">
          <div className="p-2 border-b border-gray-200">
            <input
              ...
              placeholder="Suchen…"
              className="block w-full rounded-md border border-gray-300 bg-white text-gray-900 px-2 py-1.5 focus:outline-none focus:ring-2 focus:ring-[var(--admin-sidebar-bg)] focus:border-[var(--admin-sidebar-bg)] text-sm"
            />
          </div>
          <div className="max-h-56 overflow-auto py-1">
            ...
            {filtered.length === 0 && (
              <div className="px-3 py-2 text-xs text-gray-500">Keine Treffer</div>
            )}
          </div>
        </div>
      )}
